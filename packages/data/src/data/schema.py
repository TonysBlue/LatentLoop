...
class TrajectoryIdentity:
    ...
    def __post_init__(self) -> None:
        if self.schema_version != SCHEMA_VERSION:
            raise ValueError("trajectory data must use schema v5")
...
def validate_metadata(metadata: dict[str, Any]) -> None:
    if int(metadata.get("schema_version", -1)) != SCHEMA_VERSION:
        raise ValueError("unsupported trajectory schema; migrate explicitly")
    required = ("dataset_scale", "sample_kind", "task_id", "environment_id", "environment_version")
    missing = [key for key in required if key not in metadata]
    if missing:
        raise ValueError(f"schema v5 metadata is missing: {missing}")
