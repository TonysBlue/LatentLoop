...
│   ├── training/      # Pretrain/SFT/Online Recurrent PPO、checkpoint、评估
...
Canary、Pilot、Production、Smoke 和 direct-speech gate 使用同一个 train/recipe/evaluate 代码路径。正式规模均按 `Pretrain -> SFT -> Online Recurrent PPO` 运行；差异只来自配置中的数据量、更新数、PPO window、环境并发和资源预算，不得添加阶段专用 Python 或 shell 训练循环。Pretrain/SFT 使用共享监督分支，PPO 使用共享在线环境分支，但两者仍由同一个 `training.py::train` 按 objective 分派。
