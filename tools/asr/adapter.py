...
def main() -> None:
    ...
    if request.get("operation") != "transcribe":
        raise ValueError("unsupported ASR operation")
    payload = json.dumps({"request": request, "output": str(args.output)}).encode()
    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as connection:
        connection.settimeout(300)
        connection.connect(os.environ["SENSEVOICE_SOCKET"])
    ...
    if not response.get("ok"):
        raise RuntimeError(str(response.get("error", "SenseVoice worker failed")))
